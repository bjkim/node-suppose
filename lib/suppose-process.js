...
Suppose.prototype.logger = function (logger) {
  this.logger = logger; 
  return this;
}
...
Suppose.prototype.end = function(callback){
  ...
  if (self.logger) {
    cmdString = util.format('%s %s', this.cmd, this.args.join(' '));
    self.logger.info(cmdString.toString('utf8'));
  }
  ...
  exe.stdout.on('data', function(data){
    ...
    if (match) {
      ...
      if (self.logger) {
        self.logger.info(response.toString('utf8'));
      }
